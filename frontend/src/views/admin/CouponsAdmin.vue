...
const createCoupon = async () => {
  ...
  try {
    ...
    alert(
      "Không tạo được coupon: " + (err.response?.data?.message || err.message),
    );
  } finally {
    saving.value = false;
  }
...
const deleteRow = async (row) => {
  ...
  try {
    ...
    alert(
      "Không xóa được coupon: " + (err.response?.data?.message || err.message),
    );
  }
};
  ...
  <div class="coupon-admin">
    <div class="head">
      <h2 style="font-size: 1.75rem; font-weight: 700">
        Quản lý <span class="gradient-text">Coupon</span>
      </h2>
    ...
    <section class="filter-bar glass-panel">
      <input
        v-model="searchQuery"
        class="field"
        placeholder="Tìm theo mã / số tiền"
      />
    ...
    <section class="create-box glass-panel">
      ...
      <div class="form-grid">
        ...
        <input
          v-model.number="form.discount_cents"
          class="field"
          type="number"
          min="0"
        ...
        />
        <input
          v-model.number="form.min_subtotal_cents"
          class="field"
          type="number"
          min="0"
          placeholder="Min subtotal (cents)"
        />
        <input
      ...
      <button class="btn" :disabled="saving" @click="createCoupon">
        Tao coupon
      </button>
    </section>
    ...
    <table v-else class="table glass-panel">
      ...
      <tbody>
        <tr v-for="row in rows" :key="row.id">
          ...
          <td class="actions">
            <button class="btn-small" @click="toggleActive(row)">
